...
contract RouletteMock is VRFConsumerBaseV2{
    ...
    function placeBet(uint256[] memory _numbersSelected) public payable onlyValidUserStake(msg.value) onlyRegistered(msg.sender) onlyValidHouseFunds(msg.value){
        //should freeze any more functionality instantly actually (like dont let the user button spam)
        ...
        require (verifyUser(msg.sender), "User has not been registered with the casino");

        // This line has also been added since normally the coordinator would return a request ID. The original line is below
        ...
        // This part has been added, normally would call the coordinator function and allow that to call the fulfillRandomWords function and the next 3 lines are not in the contract
